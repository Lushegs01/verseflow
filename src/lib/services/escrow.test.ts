...
async function makeFundedAgreement(
  client: User,
  provider: User,
  amounts: number[],
  ...
  const agreement = await agreementsRepo.insert({
    id: newId("agr"),
    reference: `VF-${1000 + (await agreementsRepo.nextSequence())}`,
...
async function putUnderReview(milestone: Milestone): Promise<Milestone> {
  return await milestonesRepo.update({ ...milestone, status: "under_review", submittedAt: nowIso() });
}
...
beforeEach(async () => {
  ...
  client = await makeUser("Client", CLIENT_ADDRESS);
  provider = await makeUser("Provider", PROVIDER_ADDRESS);
  vi.spyOn(console, "error").mockImplementation(() => {});
});

describe("funding", () => {
  it("funds escrow and activates the first milestone", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [75_000, 150_000, 75_000]);
    ...
    const milestones = await milestonesRepo.forAgreement(funded.id);
  ...
  it("refuses funding by anyone other than the client", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [100_000]);
    await expect(
      prepareFunding({
        bundle: await hydrate(agreement),
        actor: provider,
  ...
  it("refuses funding before both parties have signed", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [100_000]);
    const unsigned = await agreementsRepo.update({ ...agreement, providerSignature: null });

    await expect(
      prepareFunding({
        bundle: await hydrate(unsigned),
        actor: client,
        idempotencyKey: "k".repeat(12),
        fromAddress: CLIENT_ADDRESS,
      }),
  ...
  it("rejects funding when milestone amounts do not equal the total", async () => {
    const { agreement, milestones } = await makeFundedAgreement(client, provider, [100_000]);
    // Corrupt the allocation the way a tampered client would.
    await milestonesRepo.update({ ...milestones[0], amount: 90_000 });
...
describe("release authorization", () => {
  it("refuses a release initiated by the provider", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [100_000]);
    const funded = await fundIt(agreement, client);
    const milestone = await putUnderReview((await milestonesRepo.forAgreement(funded.id))[0]);

    await expect(
      releaseMilestone({
        bundle: await hydrate(funded),
        milestone,
        actor: provider,
        amount: 100_000,
  ...
  it("refuses a release by an unrelated third party", async () => {
    const stranger = await makeUser("Stranger", "0x3333333333333333333333333333333333333333");
    const { agreement } = await makeFundedAgreement(client, provider, [100_000]);
    const funded = await fundIt(agreement, client);
    const milestone = await putUnderReview((await milestonesRepo.forAgreement(funded.id))[0]);
...
describe("release amounts", () => {
  it("releases a full milestone and confirms the payment", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [75_000, 150_000]);
    const funded = await fundIt(agreement, client);
    const milestone = await putUnderReview((await milestonesRepo.forAgreement(funded.id))[0]);
    ...
    const settled = (await milestonesRepo.byId(milestone.id))!;
    expect(settled.status).toBe("released");
    expect(settled.releasedAmount).toBe(75_000);

    // The next milestone should now be active.
    expect((await milestonesRepo.forAgreement(funded.id))[1].status).toBe("in_progress");
  });
...
describe("partial release", () => {
  it("releases part and leaves the remainder locked", async () => {
    const { agreement } = await makeFundedAgreement(client, provider, [150_000]);
    const funded = await fundIt(agreement, client);
    const milestone = await putUnderReview((await milestonesRepo.forAgreement(funded.id))[0]);
    ...
    expect(await remainingFor(settled)).toBe(60_000);
  });
  ...
  it("two partial releases settle the milestone exactly", async () => {
    ...
    expect(settled.status).toBe("released");
    expect(await remainingFor(settled)).toBe(0);

    // The agreement had a single milestone, so it should now be complete.
    expect((await agreementsRepo.byId(funded.id))!.status).toBe("completed");
  });
});
...
describe("payment ledger integrity", () => {
  it("a failed payment does not credit the milestone", async () => {
    ...
    const orphaned = await paymentsRepo.update({ ...result.payment, txHash: "0x" + "9".repeat(64) });
  ...
  it("confirmed payments cannot be altered", async () => {
    ...
    const db = await getDb();
    await expect(
      db.query("UPDATE payments SET amount = $1 WHERE id = $2", [1, result.payment.id]),
    ).rejects.toThrow(/immutable|cannot be altered/i);
  });
  ...
  it("payments cannot be deleted", async () => {
    ...
    const db = await getDb();
    await expect(
      db.query("DELETE FROM payments WHERE id = $1", [result.payment.id]),
