...
function makeUser(name: string, opts: { publicProfile?: boolean; metrics?: PublicMetricKey[] } = {}): User {
 ...
 */
function settledAgreement(opts: {
  client: User;
  provider: User;
  amounts: number[];
  ...
  amounts.forEach((amount, index) => {
    ...
    const milestone = milestonesRepo.insert({
    ...
    paymentsRepo.insert({
      ...
      recipientAddress: walletsRepo.primaryAddress(provider.id)!,
...
beforeEach(() => {
  ...
  provider = makeUser("Alex Morgan");
  clientA = makeUser("Client A");
  clientB = makeUser("Client B");
  vi.spyOn(console, "error").mockImplementation(() => {});
});

describe("computeReputation", () => {
  it("is empty for a user with no history", () => {
    const r = computeReputation(provider.id);
  ...
  it("counts completed contracts and settled value", () => {
    settledAgreement({ client: clientA, provider, amounts: [75_000, 150_000], startedDaysAgo: 90, durationDays: 30 });
    settledAgreement({ client: clientB, provider, amounts: [100_000], startedDaysAgo: 50, durationDays: 20 });
  ...
  it("counts ONLY confirmed payments, never approved-but-unconfirmed", () => {
    settledAgreement({ client: clientA, provider, amounts: [100_000], startedDaysAgo: 60, durationDays: 20 });
  ...
  it("computes the on-time rate from released vs due dates", () => {
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 90, durationDays: 20, onTime: true });
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 70, durationDays: 20, onTime: true });
    settledAgreement({ client: clientB, provider, amounts: [50_000], startedDaysAgo: 50, durationDays: 20, onTime: true });
    settledAgreement({ client: clientB, provider, amounts: [50_000], startedDaysAgo: 30, durationDays: 20, onTime: false });

    expect(computeReputation(provider.id).onTimeRate).toBe(75);
  });

  it("computes the repeat-client rate from returning clients", () => {
    // clientA hires three times, clientB once -> 3 of 4 agreements are repeat.
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 120, durationDays: 20 });
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 90, durationDays: 20 });
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 60, durationDays: 20 });
    settledAgreement({ client: clientB, provider, amounts: [50_000], startedDaysAgo: 30, durationDays: 20 });

    expect(computeReputation(provider.id).repeatClientRate).toBe(75);
  });

  it("does not credit the provider for work they commissioned as a client", () => {
    // The provider hires someone else. That is not their delivery record.
    settledAgreement({ client: provider, provider: clientA, amounts: [400_000], startedDaysAgo: 40, durationDays: 15 });
  ...
  it("averages completion time across completed agreements", () => {
    settledAgreement({ client: clientA, provider, amounts: [50_000], startedDaysAgo: 100, durationDays: 20 });
    settledAgreement({ client: clientB, provider, amounts: [50_000], startedDaysAgo: 60, durationDays: 40 });

    expect(computeReputation(provider.id).avgCompletionDays).toBe(30);
  });
});

describe("public profile privacy", () => {
  it("returns null when the profile is not enabled", () => {
    settledAgreement({ client: clientA, provider, amounts: [100_000], startedDaysAgo: 40, durationDays: 20 });
    // Absence is itself private: callers render a 404, not "this user is private".
    expect(buildPublicProfile(provider.handle)).toBeNull();
  });

  it("returns null for an unknown handle", () => {
    expect(buildPublicProfile("nobody-here")).toBeNull();
  });

  it("exposes only the metrics the user opted into", () => {
    const opted = makeUser("Opted In", {
      publicProfile: true,
      metrics: ["contracts_completed", "on_time_rate"],
    });
    settledAgreement({ client: clientA, provider: opted, amounts: [100_000], startedDaysAgo: 40, durationDays: 20 });

    const profile = buildPublicProfile(opted.handle)!;
  ...
  it("publishes nothing when the profile is on but no metrics are selected", () => {
    const opted = makeUser("Bare Profile", { publicProfile: true, metrics: [] });
    settledAgreement({ client: clientA, provider: opted, amounts: [100_000], startedDaysAgo: 40, durationDays: 20 });
...
describe("showcase", () => {
  it("offers only completed agreements", () => {
    settledAgreement({ client: clientA, provider, amounts: [100_000], startedDaysAgo: 40, durationDays: 20 });
    settledAgreement({
    ...
    const candidates = showcaseCandidates(provider.id);
    expect(candidates).toHaveLength(1);
    expect(candidates[0].status).toBe("completed");
  });

  it("hides the exact value when the user anonymized it", () => {
    const opted = makeUser("Showcase User", { publicProfile: true, metrics: ["contracts_completed"] });
    const agreement = settledAgreement({
    ...
    const item = buildPublicProfile(opted.handle)!.showcase[0];
    expect(item.title).toBe("E-commerce redesign");
    expect(item.valueLabel).toBe("$1K - $5K");
    expect(item.valueLabel).not.toContain("3,200");
  });

  it("shows the exact value when the user chose to", () => {
    const opted = makeUser("Open Book", { publicProfile: true, metrics: [] });
    const agreement = settledAgreement({
      client: clientA, provider: opted, amounts: [320_000], startedDaysAgo: 40, durationDays: 20,
    });
    ...
    expect(buildPublicProfile(opted.handle)!.showcase[0].valueLabel).toBe("$3,200");
  });

  it("never publishes a showcased agreement the user is not party to", () => {
    const opted = makeUser("Sneaky", { publicProfile: true, metrics: [] });
    // An agreement between two other people.
    const foreign = settledAgreement({
    ...
    expect(buildPublicProfile(opted.handle)!.showcase).toHaveLength(0);
  });
});
